...
import itertools
from src.utils import bitstrings, xor, list_of_bool_to_binary_string


def draw_graph(graph: nx.Graph, fname: str = "test.svg", **kwargs):
...
def draw_assignments(assignments, phi=None):
    ...
    for x, y in itertools.combinations(assignments, 2):
        ...
        diff = xor(x, y)
        if (sum(diff) == 1):
            G.add_edge(list_of_bool_to_binary_string(x),list_of_bool_to_binary_string(y), label=str(diff.index(1)+1))
    draw_graph(G)
